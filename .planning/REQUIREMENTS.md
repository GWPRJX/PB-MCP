# Requirements: PB MCP

**Defined:** 2026-03-19
**Core Value:** Small business owners and their AI assistants can manage real ERP operations — checking stock, creating invoices, looking up customers — without writing code, while developers can add new tenants and go live in under 10 minutes.

## v2.1 Requirements

Requirements for v2.1 release. Each maps to roadmap phases.

### KB/Docs Management

- [x] **KB-01**: Admin can configure YouTrack connection settings (API key, base URL, project) from the dashboard
- [x] **KB-02**: Admin can trigger manual YouTrack sync from dashboard via sync button
- [x] **KB-03**: Dashboard shows last sync timestamp, article count, and success/failure status
- [x] **KB-04**: Admin can manually upload API docs at server level to fill gaps not covered by YouTrack sync
- [x] **KB-05**: Uploaded docs are searchable via MCP KB tools alongside YouTrack-synced articles

### Tenant Onboarding

- [x] **ONBOARD-01**: Tenant creation flow prompts for ERP credentials first
- [x] **ONBOARD-02**: ERP connection is tested before proceeding to API key generation
- [x] **ONBOARD-03**: API key is only generated after successful ERP connection test

### Dashboard UX

- [x] **UX-01**: Dashboard shows MCP client setup instructions (URL, API key, where to add them)
- [x] **UX-02**: Setup instructions cover Claude Desktop, Cursor, and generic MCP client
- [x] **UX-03**: Admin can export per-tenant branded PDF with setup + usage instructions (tenant name, MCP URL, masked API key)
- [x] **UX-04**: Tooltips added for all technical terms (API key, slug, tool permissions, ERP config, etc.)

### Documentation

- [x] **DOCS-01**: README overhauled with clear project overview and quickstart
- [x] **DOCS-02**: Step-by-step setup guide for Linux/VPS (Ubuntu/Debian + Node.js + PostgreSQL)
- [x] **DOCS-03**: Step-by-step setup guide for Docker (Dockerfile + docker-compose)
- [x] **DOCS-04**: Step-by-step setup guide for Windows Server

## Future Requirements

None — v2.1 is a polish/documentation milestone.

## Out of Scope

| Feature | Reason |
|---------|--------|
| New MCP tools | No new tools in v2.1 — this is UX/docs only |
| Database schema changes | No data model changes needed |
| Multi-language docs | English only for now |
| Video tutorials | Written guides sufficient for v2.1 |

## Traceability

| Requirement | Phase | Status |
|-------------|-------|--------|
| KB-01 | Phase 8 | Complete |
| KB-02 | Phase 8 | Complete |
| KB-03 | Phase 8 | Complete |
| KB-04 | Phase 8 | Pending |
| KB-05 | Phase 8 | Pending |
| ONBOARD-01 | Phase 9 | Complete |
| ONBOARD-02 | Phase 9 | Complete |
| ONBOARD-03 | Phase 9 | Complete |
| UX-01 | Phase 10 | Complete |
| UX-02 | Phase 10 | Complete |
| UX-03 | Phase 10 | Complete |
| UX-04 | Phase 10 | Complete |
| DOCS-01 | Phase 11 | Complete |
| DOCS-02 | Phase 11 | Complete |
| DOCS-03 | Phase 11 | Complete |
| DOCS-04 | Phase 11 | Complete |

**Coverage:**
- v2.1 requirements: 16 total
- Mapped to phases: 16
- Unmapped: 0

---
*Requirements defined: 2026-03-19*
*Last updated: 2026-03-19 after Phase 8 scope revision — 16 requirements mapped*
